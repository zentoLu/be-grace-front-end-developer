    ...
    var quickFlip = {
        ...
        createFlip: function(x, y, width) {
            ...
            for (z = 0; z < 2; z++) {
                ...
                out += '<div class="' + theClass + '" style="width: ' + width + 'px; height: ' + x.height + '; ' + thePos + ': ' + x.halfwidth + 'px;"><div class="' + x.classNames[y] + '">' + x.panels[y].clone().find('.not.flip').remove().end().html() + '</div></div>'
            }
            return out;
        },
                        ...
                        if (!count2) count2++;
                            ...
                            switch (repeater) {
                                ...
                                case 1: //stop if is last flip, and attach events for msie
                                    ...
                                    //     var $quickFlipCta = $('.quickFlipCta', x.panels[k]);
                                    //     if($quickFlipCta.length === 0){
                                    //         $quickFlipCta = $(x.panels[k]);
                                    //     }
                                    //     quickFlip.attachHandlers($quickFlipCta, i, k);
                                    // }
        ...
        init: function($el, befornFlipFn, afterFlipFn) {
            // quickFlip.wrappers = new Array();

            $el = $el.filter(':not(.init-flip)');
            if($el.length === 0){
                return;
            }
            if(befornFlipFn && $.isFunction(befornFlipFn)){
                $el.data('befornFlipFn', befornFlipFn);
            }
            if(afterFlipFn && $.isFunction(afterFlipFn)){
                $el.data('afterFlipFn', afterFlipFn);
            }
            ...
            $el.each(function(i) {
                ...
                $this.children('.quickFlipPanel').each(function(j) {
                    ...
                    var $quickFlipCta = $('.quickFlipCta', $thisPanel);
                    if($quickFlipCta.length === 0){
                        $quickFlipCta = $thisPanel;
                    }
                    quickFlip.attachHandlers($quickFlipCta, i, j);
            ...
            $el.addClass('init-flip');
        }
    };
    $.fn.quickflip = function(){
        // todo befornFlipFn, afterFlipFn
        quickFlip.init(this);
    }
});
